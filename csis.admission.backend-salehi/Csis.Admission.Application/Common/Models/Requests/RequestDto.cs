...
/// <inheritdoc/>
public record RequestDto : BaseDto<RequestDto, Request, long>
{
    /// <inheritdoc/>
    ...
    /// <inheritdoc/>
    public override void CustomMappings(IMappingExpression<Request, RequestDto> mapping) {
        ...
        mapping.ForMember(dto => dto.StudentApprovers, config => config.MapFrom(model => model.Approvers.Where(x => x.Status == Domain.Enums.ApprovalStatus.Pending).Select(x => $"({x.ApproverCodm}) {x.Approver}").ToArray()));
