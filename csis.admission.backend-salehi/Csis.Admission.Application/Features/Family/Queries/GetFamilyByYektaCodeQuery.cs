...
/// <summary>
/// اطلاعات افراد و تکفل براساس کد یکتا
/// </summary>
/// <param name="YektaCode"></param>
public sealed record GetFamilyByYektaCodeQuery(string YektaCode) : IRequest<List<HealthInsuranceFamilyDto>>;

internal sealed class GetFamilyByYektaCodeQueryHandler : IRequestHandler<GetFamilyByYektaCodeQuery, List<HealthInsuranceFamilyDto>>
{
    ...
    public GetFamilyByYektaCodeQueryHandler(IMapper mapper
    ...
    public async Task<List<HealthInsuranceFamilyDto>> Handle(GetFamilyByYektaCodeQuery request, CancellationToken cancellationToken) {


        var student = await _studentRepo.GetAllAsync(i => i.YektaCode == request.YektaCode);
        ...
        var dependent = await _dependentRepo.GetAllAsync(i => i.YektaCode == request.YektaCode);
