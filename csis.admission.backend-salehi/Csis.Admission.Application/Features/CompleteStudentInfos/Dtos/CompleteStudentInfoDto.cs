...
namespace Csis.Admission.Application.Features.CompleteStudentInfos.Dtos;

/// <summary>اطلاعات کامل طلبه</summary>
public record CompleteStudentInfoDto : BaseDto<CompleteStudentInfoDto, CompleteStudentInfo>
{
    ...
    public string? PreachLastAcitvity { get; set; }
    ...
    /// <inheritdoc/>
    public override void CustomMappings(IMappingExpression<CompleteStudentInfo, CompleteStudentInfoDto> mapping) {
        mapping.ForMember(dto => dto.Age, config => config.MapFrom(model => Common.Utilities.CalculateAge(model.BirthDate, model.DeathDate)));
        mapping.ForMember(dto => dto.PreachLastAcitvity, config => config.MapFrom(model => model.PreachLastAcitvity.HasValue ? model.PreachLastAcitvity.Value.IntDateToString() : null));
    }
}
