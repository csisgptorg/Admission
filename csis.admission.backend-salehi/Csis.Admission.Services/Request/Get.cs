...
internal sealed partial class RequestService : IRequestService
{
    ...
    public async Task<RequestDto> GetById(long id, CancellationToken cancellationToken) {
        var result = await repo.GetByIdAsync<RequestDto>(id, false, cancellationToken: cancellationToken);
    ...
    public async Task<List<RequestDto>> GetAllByCodmAsync(int? codm, bool? isCompleted, CancellationToken cancellationToken) {
        if ( codm == null ) {
            codm = int.Parse(await authenticatedUser.GetStudentCodmAsync());
        }
        var result = await repo.GetAllAsync<RequestDto>(x =>
            x.Codm == codm &&
            (
                isCompleted == null ||
    ...
    public async Task<IPagedList<RequestDto>> GetAllAsync(AllRequestQuery query, CancellationToken cancellationToken) {

        var result = await repo.GetAllPagedAsync<RequestDto>(
           query.PageIndex,
            query.PageSize,
            sortExpression: "-Id",
    ...
    private async Task<EmployeeDto> CurrentEmployee() {
        ...
        try {
            ...
            return new EmployeeDto(personnelId.Value, fullName, isSenior);
        } catch ( Exception e ) {
            return new EmployeeDto(personnelId.Value, "فاقد مشخصات", false);
        }
